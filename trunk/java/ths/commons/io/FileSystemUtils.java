...
public class FileSystemUtils {
	 ...
	 */
	public static boolean copyFile(File src, File destDirectory, String filename, boolean coverlay) {
		if (!isValid(src) || !isValid(destDirectory) || !src.isFile() || !src.canRead() || !destDirectory.isDirectory()
				|| !destDirectory.canWrite()) {
			return false;
		}
	 ...
	 */
	public static boolean copyFile(File src, File destDirectory, boolean coverlay) {
		return copyFile(src, destDirectory, null, coverlay);
	}
	...
	private static void copyRecursively(File src, File dest) throws IOException {
		if (src.isFile()) {
			if (!copyFile(src, dest.getParentFile(), null, false)) {
	...
	public static boolean deleteDirectory(File root) {
		if (!isValid(root) || !root.isDirectory() || !root.canWrite()) {
			return false;
		}
