...
import ths.commons.util.ClassLoaderUtils;
...
public class LocaleUtils {
    ...
    private static Notifier[] getNotifiers() {
        try {
            URL[] files = ClassLoaderUtils.getResources("META-INF/services/localeNotifiers", ClassLoaderUtils.class);
            List<Notifier> list = new LinkedList<Notifier>();

            for (URL file : files) {
                for (String className : split(StreamUtils.readText(file.openStream(), "UTF-8", true), "\r\n ")) {
                    list.add(Notifier.class.cast(ClassLoaderUtils.newInstance(className, ClassLoaderUtils.class)));
                }
            }
    ...
    private static class AvailableLocales {
        private final Set<String> AVAILABLE_LANGUAGES = new HashSet<String>();
        private final Set<String> AVAILABLE_COUNTRIES = new HashSet<String>();
