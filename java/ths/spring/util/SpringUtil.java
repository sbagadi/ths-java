...
import ths.spring.BeanElement;
import ths.spring.ProxyTargetFactory;
import ths.spring.SingleNamespace;
import ths.spring.util.DomUtil;
import ths.spring.util.DomUtil.ElementSelector;
import ths.commons.util.ArrayUtils;
...
public class SpringUtil {
    ...
    public static SingleNamespace getSiblingCustomNamespace(String customNamespace, BeanElement element) {
        assertNotNull(element, "beanElement");
        return getSiblingCustomNamespace(customNamespace, element.getSingleNamespace());
    }

    public static SingleNamespace getSiblingCustomNamespace(String customNamespace, SingleNamespace cn) {
        assertNotNull(customNamespace, "customNamespace");
        assertNotNull(cn, "configurationPoint");

        SingleNamespace siblingCn = cn.getServicesNamespace().getSingleNamespaceByName(customNamespace);

        assertNotNull(siblingCn, "could not find configuration point of name: %s", customNamespace);
    ...
    public static Set<Object> createManagedSet(Element element, ParserContext parserContext) {
        ManagedSet<Object> set = new ManagedSet<Object>();
    ...
    public static void elementToProperty(Element element, String propertyPrefix, BeanDefinitionBuilder builder) {
        ...
        builder.addPropertyValue(propName, StringUtils.trimToNull(element.getTextContent()));
    }
    ...
    public static void attributesToProperties(Element element, String propertyPrefix, BeanDefinitionBuilder builder,
        ...
        HashSet<String> attrNameSet = ArrayUtils.isEmpty(attrNames) ? null : new HashSet<String>(attrNames.length);
    ...
    public static void attributeToProperty(Attr attr, String propertyPrefix, BeanDefinitionBuilder builder) {
        ...
        builder.addPropertyValue(propName, StringUtils.trimToNull(attr.getNodeValue()));
    }
     ...
     */
    public static BeanDefinitionHolder parseConfigurationPointBean(Element element, SingleNamespace cn, ParserContext parserContext, BeanDefinitionBuilder containingBeanBuilder) {
        assertNotNull(cn, "customNamespace");
        ...
        ElementSelector customSelector = DomUtil.ns(cn.getNamespaceUri());
        ...
        if (customSelector.accept(element)) {
            ...
            String beanName = StringUtils.trimToNull(element.getAttribute("id"));
    ...
    public static Object parseBean(Element element, ParserContext parserContext, BeanDefinition containingBean) {
        BeanDefinitionParserDelegate delegate = parserContext.getDelegate();
        String refName = StringUtils.trimToNull(element.getAttribute("ref"));
    ...
    public static BeanDefinition createOptionalPropertyRef(String beanName, Class<?> beanType) {
        beanName = assertNotNull(StringUtils.trimToNull(beanName), "beanName");
        assertNotNull(beanType, "beanType");
    ...
    public static class ConstructorArg implements FactoryBean<Object>, BeanFactoryAware {
    ...
    public static class OptionalPropertyRef implements FactoryBean<Object>, BeanFactoryAware {
    ...
    private static final class ProxiedFilter implements CallbackFilter {
        public int accept(Method method) {
            if (ReflectionUtils.isEqualsMethod(method) || ReflectionUtils.isHashCodeMethod(method) || ReflectionUtils.isToStringMethod(method)
